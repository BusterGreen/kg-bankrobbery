...
local function CheckStationHits()
    ...
    if table.type(bankHits) == 'array' then TriggerClientEvent('kg-bankrobbery:client:BankSecurity', -1, bankHits, false) end
end
...
RegisterNetEvent('kg-bankrobbery:server:setBankState', function(bankId)
    ...
    if bankId == 'paleto' then
        if Config.BigBanks['paleto']['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['paleto']['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:setBankState', extraInfo = ' (paleto) ', source = source }))
        end
        Config.BigBanks['paleto']['isOpened'] = true
        TriggerEvent('kg-bankrobbery:server:setTimeout')
    elseif bankId == 'pacific' then
        if Config.BigBanks['pacific']['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['pacific']['coords'][2]) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:setBankState', extraInfo = ' (pacific) ', source = source }))
        end
        ...
        if Config.SmallBanks[bankId]['isOpened'] or #(GetEntityCoords(GetPlayerPed(source)) - Config.SmallBanks[bankId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:setBankState', extraInfo = ' (smallbank ' .. bankId .. ') ', source = source }))
        end
        Config.SmallBanks[bankId]['isOpened'] = true
        TriggerEvent('kg-bankrobbery:server:SetSmallBankTimeout', bankId)
    end
    TriggerClientEvent('kg-bankrobbery:client:setBankState', -1, bankId)
    robberyBusy = true
    Config.OnRobberyStart(bankId)
end)

RegisterNetEvent('kg-bankrobbery:server:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == 'paleto' or bankId == 'pacific' then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks[bankId]['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:setLockerState', extraInfo = ' (' .. bankId .. ') ', source = source }))
        end
        ...
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.SmallBanks[bankId]['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:setLockerState', extraInfo = ' (smallbank ' .. bankId .. ') ', source = source }))
        end
        Config.SmallBanks[bankId]['lockers'][lockerId][state] = bool
    end
    TriggerClientEvent('kg-bankrobbery:client:setLockerState', -1, bankId, lockerId, state, bool)
end)

RegisterNetEvent('kg-bankrobbery:server:recieveItem', function(type, bankId, lockerId)
    local src = source
    local ply = KGCore.Functions.GetPlayer(src)
    if not ply then return end
    if type == 'small' then
        if #(GetEntityCoords(GetPlayerPed(src)) - Config.SmallBanks[bankId]['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:receiveItem', extraInfo = ' (smallbank ' .. bankId .. ') ', source = source }))
        end
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                if Config.RewardTypes[itemType].type == 'item' then
                    ...
                    exports['kg-inventory']:AddItem(src, item.item, itemAmount, false, false, 'kg-bankrobbery:server:recieveItem')
                    TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items[item.item], 'add')
                elseif Config.RewardTypes[itemType].type == 'money' then
                    local info = {
                        worth = math.random(2300, 3200)
                    }
                    exports['kg-inventory']:AddItem(src, 'markedbills', math.random(2, 3), false, info, 'kg-bankrobbery:server:recieveItem')
                    TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['markedbills'], 'add')
                end
            else
                exports['kg-inventory']:AddItem(src, 'security_card_01', 1, false, false, 'kg-bankrobbery:server:recieveItem')
                TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['security_card_01'], 'add')
            end
        else
            exports['kg-inventory']:AddItem(src, 'weapon_stungun', 1, false, false, 'kg-bankrobbery:server:recieveItem')
            TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['weapon_stungun'], 'add')
        end
    elseif type == 'paleto' then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['paleto']['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:receiveItem', extraInfo = ' (paleto) ', source = source }))
        end
        local itemType = math.random(#Config.RewardTypes)
        local tierChance = math.random(1, 100)
        ...
        if WeaponChance ~= odd1 then
            if tier ~= 4 then
                ...
                elseif Config.RewardTypes[itemType].type == 'money' then
                    ...
                    exports['kg-inventory']:AddItem(src, 'markedbills', math.random(1, 4), false, info, 'kg-bankrobbery:server:recieveItem')
                    TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['markedbills'], 'add')
                end
            else
                exports['kg-inventory']:AddItem(src, 'security_card_02', 1, false, false, 'kg-bankrobbery:server:recieveItem')
                TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['security_card_02'], 'add')
            end
        else
            exports['kg-inventory']:AddItem(src, 'weapon_vintagepistol', 1, false, false, 'kg-bankrobbery:server:recieveItem')
            TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['weapon_vintagepistol'], 'add')
        end
    elseif type == 'pacific' then
        if #(GetEntityCoords(GetPlayerPed(source)) - Config.BigBanks['pacific']['lockers'][lockerId]['coords']) > 2.5 then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:receiveItem', extraInfo = ' (pacific) ', source = source }))
        end
        local itemType = math.random(#Config.RewardTypes)
        ...
        if WeaponChance ~= odd1 or WeaponChance ~= odd2 then
            if tier ~= 4 then
                ...
                exports['kg-inventory']:AddItem(src, 'cryptostick', 1, false, info, 'kg-bankrobbery:server:recieveItem')
            ...
            if chance == odd then
                exports['kg-inventory']:AddItem(src, 'weapon_microsmg', 1, false, false, 'kg-bankrobbery:server:recieveItem')
                TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['weapon_microsmg'], 'add')
            else
                exports['kg-inventory']:AddItem(src, 'weapon_minismg', 1, false, false, 'kg-bankrobbery:server:recieveItem')
                TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['weapon_minismg'], 'add')
            end
        end
    end
end)

AddEventHandler('kg-bankrobbery:server:setTimeout', function()
    ...
    CreateThread(function()
        SetTimeout(60000 * 90, function()
            ...
            TriggerClientEvent('kg-bankrobbery:client:ClearTimeoutDoors', -1)
...
AddEventHandler('kg-bankrobbery:server:SetSmallBankTimeout', function(bankId)
    if robberyBusy or timeOut then return end
    timeOut = true
    CreateThread(function()
        SetTimeout(60000 * 30, function()
            ...
            TriggerClientEvent('kg-bankrobbery:client:ResetFleecaLockers', -1, bankId)
            timeOut = false
            robberyBusy = false
...
RegisterNetEvent('kg-bankrobbery:server:callCops', function(type, bank, coords)
    if type == 'small' then
        if not Config.SmallBanks[bank]['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:callCops', extraInfo = ' (smallbank ' .. bank .. ') ', source = source }))
        end
    elseif type == 'paleto' then
        if not Config.BigBanks['paleto']['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:callCops', extraInfo = ' (paleto) ', source = source }))
        end
    elseif type == 'pacific' then
        if not Config.BigBanks['pacific']['alarm'] then
            return error(Lang:t('error.event_trigger_wrong', { event = 'kg-bankrobbery:server:callCops', extraInfo = ' (pacific) ', source = source }))
        end
    end
    TriggerClientEvent('kg-bankrobbery:client:robberyCall', -1, type, coords)
end)

RegisterNetEvent('kg-bankrobbery:server:SetStationStatus', function(key, isHit)
    Config.PowerStations[key].hit = isHit
    TriggerClientEvent('kg-bankrobbery:client:SetStationStatus', -1, key, isHit)
    if AllStationsHit() then
        exports['kg-weathersync']:setBlackout(true)
        TriggerClientEvent('kg-bankrobbery:client:disableAllBankSecurity', -1)
        Config.OnBlackout(true)
        CreateThread(function()
            SetTimeout(60000 * Config.BlackoutTimer, function()
                exports['kg-weathersync']:setBlackout(false)
                TriggerClientEvent('kg-bankrobbery:client:enableAllBankSecurity', -1)
...
RegisterNetEvent('kg-bankrobbery:server:removeElectronicKit', function()
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if not Player then return end
    exports['kg-inventory']:RemoveItem(src, 'electronickit', 1, false, 'kg-bankrobbery:server:removeElectronicKit')
    TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['electronickit'], 'remove')
    exports['kg-inventory']:RemoveItem(src, 'trojan_usb', 1, false, 'kg-bankrobbery:server:removeElectronicKit')
    TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['trojan_usb'], 'remove')
end)

RegisterNetEvent('kg-bankrobbery:server:removeBankCard', function(number)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if not Player then return end
    exports['kg-inventory']:RemoveItem(src, 'security_card_' .. number, 1, false, 'kg-bankrobbery:server:removeBankCard')
    TriggerClientEvent('kg-inventory:client:ItemBox', src, KGCore.Shared.Items['security_card_' .. number], 'remove')
end)
...
KGCore.Functions.CreateCallback('kg-bankrobbery:server:isRobberyActive', function(_, cb)
    cb(robberyBusy)
end)

KGCore.Functions.CreateCallback('kg-bankrobbery:server:GetConfig', function(_, cb)
    cb(Config.PowerStations, Config.BigBanks, Config.SmallBanks)
end)

KGCore.Functions.CreateCallback('thermite:server:check', function(source, cb)
    local Player = KGCore.Functions.GetPlayer(source)
    if not Player then return cb(false) end
    if exports['kg-inventory']:RemoveItem(source, 'thermite', 1, false, 'thermite:server:check') then
        TriggerClientEvent('kg-inventory:client:ItemBox', source, KGCore.Shared.Items['thermite'], 'remove')
        cb(true)
    else
        cb(false)
    end
...
KGCore.Functions.CreateUseableItem('thermite', function(source)
    local Player = KGCore.Functions.GetPlayer(source)
    ...
    if Player.Functions.GetItemByName('lighter') then
        ...
        TriggerClientEvent('KGCore:Notify', source, Lang:t('error.missing_ignition_source'), 'error')
    end
end)

KGCore.Functions.CreateUseableItem('security_card_01', function(source)
    local Player = KGCore.Functions.GetPlayer(source)
    if not Player or not Player.Functions.GetItemByName('security_card_01') then return end
    TriggerClientEvent('kg-bankrobbery:UseBankcardA', source)
end)

KGCore.Functions.CreateUseableItem('security_card_02', function(source)
    local Player = KGCore.Functions.GetPlayer(source)
    if not Player or not Player.Functions.GetItemByName('security_card_02') then return end
    TriggerClientEvent('kg-bankrobbery:UseBankcardB', source)
end)

KGCore.Functions.CreateUseableItem('electronickit', function(source)
    local Player = KGCore.Functions.GetPlayer(source)
