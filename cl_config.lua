...
-- This is the handler for the cop count, you can change this to anything you want as this is by default the kg-policejob event
...
function Config.DoorlockAction(doorId, setLocked)
    TriggerServerEvent('kg-doorlock:server:updateState', doorId, setLocked, false, false, true, false, false)
end
...
function Config.OnEvidence(pos, chance)
    if math.random(1, 100) > chance or KGCore.Functions.IsWearingGloves() then return end
...
function Config.ShowRequiredItems(items, show)
    TriggerEvent('kg-inventory:client:requiredItems', items, show)
end
