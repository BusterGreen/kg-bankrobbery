...
RegisterNetEvent('KGCore:Client:OnPlayerLoaded', function()
    KGCore.Functions.TriggerCallback('kg-bankrobbery:server:GetConfig', function(config1, config2, config3)
...
function openLocker(bankId, lockerId) -- Globally Used
    ...
    TriggerServerEvent('kg-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', true)
    if bankId == "paleto" then
        ...
        if hasItem then
            ...
            IsDrilling = true
            KGCore.Functions.Progressbar("open_locker_drill", Lang:t("general.breaking_open_safe"), math.random(18000, 30000), false, true, {
                ...
                TriggerServerEvent('kg-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                TriggerServerEvent('kg-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                TriggerServerEvent('kg-bankrobbery:server:recieveItem', 'paleto', bankId, lockerId)
                KGCore.Functions.Notify(Lang:t("success.success_message"), "success")
                ...
                DetachEntity(DrillObject, true, true)
                DeleteObject(DrillObject)
                KGCore.Functions.Notify(Lang:t("error.cancel_message"), "error")
                SetTimeout(500, function()
                    IsDrilling = false
                end)
            end)
        else
            KGCore.Functions.Notify(Lang:t("error.safe_too_strong"), "error")
            TriggerServerEvent('kg-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
        end
    elseif bankId == "pacific" then
        ...
        if hasItem then
            ...
            KGCore.Functions.Progressbar("open_locker_drill", Lang:t("general.breaking_open_safe"), math.random(18000, 30000), false, true, {
                ...
                TriggerServerEvent('kg-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
                TriggerServerEvent('kg-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                TriggerServerEvent('kg-bankrobbery:server:recieveItem', 'pacific', bankId, lockerId)
                KGCore.Functions.Notify(Lang:t("success.success_message"), "success")
                SetTimeout(500, function()
                    IsDrilling = false
                end)
        ...
        KGCore.Functions.Progressbar("open_locker", Lang:t("general.breaking_open_safe"), math.random(27000, 37000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent('kg-bankrobbery:server:setLockerState', bankId, lockerId, 'isOpened', true)
            TriggerServerEvent('kg-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            TriggerServerEvent('kg-bankrobbery:server:recieveItem', 'small', bankId, lockerId)
            KGCore.Functions.Notify(Lang:t("success.success_message"), "success")
            SetTimeout(500, function()
                IsDrilling = false
            end)
...
RegisterNetEvent('electronickit:UseElectronickit', function()
    ...
    KGCore.Functions.TriggerCallback('kg-bankrobbery:server:isRobberyActive', function(isBusy)
        if not isBusy then
                ...
                if not Config.SmallBanks[closestBank]["isOpened"] then
                    ...
                    if hasItem then
                        Config.ShowRequiredItems({
                            [1] = {name = KGCore.Shared.Items["electronickit"]["name"], image = KGCore.Shared.Items["electronickit"]["image"]},
                            [2] = {name = KGCore.Shared.Items["trojan_usb"]["name"], image = KGCore.Shared.Items["trojan_usb"]["image"]}
                        }, false)
                        loadAnimDict("anim@gangops@facility@servers@")
                        TaskPlayAnim(ped, 'anim@gangops@facility@servers@', 'hotwire', 3.0, 3.0, -1, 1, 0, false, false, false)
                        KGCore.Functions.Progressbar("hack_gate", Lang:t("general.connecting_hacking_device"), math.random(5000, 10000), false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            StopAnimTask(ped, "anim@gangops@facility@servers@", "hotwire", 1.0)
                            TriggerServerEvent('kg-bankrobbery:server:removeElectronicKit')
                            local success = exports['kg-minigames']:Hacking(5, 30) -- code block size & seconds to solve
                            if success then
                                TriggerServerEvent('kg-bankrobbery:server:setBankState', closestBank)
                            end
                            if copsCalled or not Config.SmallBanks[closestBank]["alarm"] then return end
                            TriggerServerEvent("kg-bankrobbery:server:callCops", "small", closestBank, pos)
                            copsCalled = true
                        ...
                        KGCore.Functions.Notify(Lang:t("error.missing_item"), "error")
                    end
                else
                    KGCore.Functions.Notify(Lang:t("error.bank_already_open"), "error")
                end
            else
                KGCore.Functions.Notify(Lang:t("error.minimum_police_required", {police = Config.MinimumFleecaPolice}), "error")
            end
        else
            KGCore.Functions.Notify(Lang:t("error.security_lock_active"), "error", 5500)
        end
    end)
end)

RegisterNetEvent('kg-bankrobbery:client:setBankState', function(bankId)
    if bankId == "paleto" then
...
RegisterNetEvent('kg-bankrobbery:client:enableAllBankSecurity', function()
    for k in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = true
    end
end)

RegisterNetEvent('kg-bankrobbery:client:disableAllBankSecurity', function()
    for k in pairs(Config.SmallBanks) do
        Config.SmallBanks[k]["alarm"] = false
    end
end)

RegisterNetEvent('kg-bankrobbery:client:BankSecurity', function(key, status)
    ...
    elseif type(key) == 'number' then
        ...
        error(Lang:t("error.wrong_type", {receiver = 'kg-bankrobbery:client:BankSecurity', argument = "key", receivedType = type(key), receivedValue = key, expected = "table/array"}))
    end
end)

RegisterNetEvent('kg-bankrobbery:client:setLockerState', function(bankId, lockerId, state, bool)
    if bankId == "paleto" then
...
RegisterNetEvent('kg-bankrobbery:client:ResetFleecaLockers', function(BankId)
...
RegisterNetEvent('kg-bankrobbery:client:robberyCall', function(type, coords)
    if not isLoggedIn then return end
    local PlayerJob = KGCore.Functions.GetPlayerData().job
    ...
    for i = 1, #Config.SmallBanks do
        ...
        for k in pairs(Config.SmallBanks[i]["lockers"]) do
            if Config.UseTarget then
                exports['kg-target']:AddBoxZone('fleeca_'..i..'_coords_locker_'..k, Config.SmallBanks[i]["lockers"][k]["coords"], 1.0, 1.0, {
                ...
                lockerZone:onPlayerInOut(function(inside)
                    if inside and closestBank ~= 0 and not IsDrilling and Config.SmallBanks[i]["isOpened"] and not Config.SmallBanks[i]["lockers"][k]["isOpened"] and not Config.SmallBanks[i]["lockers"][k]["isBusy"] then
                        exports['kg-core']:DrawText(Lang:t("general.break_safe_open_option_drawtext"), 'right')
                        currentLocker = k
                    else
