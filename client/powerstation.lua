...
local currentFires = {}
local currentGate = 0
local requiredItems = {[1] = {name = KGCore.Shared.Items["thermite"]["name"], image = KGCore.Shared.Items["thermite"]["image"]}}
        ...
        if dist < 1.5 then
            if CurrentCops >= Config.MinimumThermitePolice then
                if not Config.PowerStations[closestStation].hit then
                    ...
                    KGCore.Functions.Notify(Lang:t("error.fuses_already_blown"), "error")
                end
            else
                KGCore.Functions.Notify(Lang:t("error.minium_police_required", {police = Config.MinimumThermitePolice}), "error")
            end
        end
...
RegisterNetEvent('kg-bankrobbery:client:SetStationStatus', function(key, isHit)
...
RegisterNUICallback('thermitefailed', function(_, cb)
    KGCore.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
...
RegisterNUICallback('thermitesuccess', function(_, cb)
    KGCore.Functions.TriggerCallback("thermite:server:check", function(success)
        if success then
            ...
            local coords = GetEntityCoords(PlayerPedId())
            while time > 0 do
                KGCore.Functions.Notify(Lang:t("general.thermite_detonating_in_seconds", {time = time}))
                Wait(1000)
                time -= 1
            end
            local randTime = math.random(10000, 15000)
            ...
            if currentStation ~= 0 then
                KGCore.Functions.Notify(Lang:t("success.fuses_are_blown"), "success")
                TriggerServerEvent("kg-bankrobbery:server:SetStationStatus", currentStation, true)
            elseif currentGate ~= 0 then
                KGCore.Functions.Notify(Lang:t("success.door_has_opened"), "success")
