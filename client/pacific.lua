...
RegisterNetEvent('kg-bankrobbery:UseBankcardB', function()
            ...
            if CurrentCops >= Config.MinimumPacificPolice then
                if not Config.BigBanks["pacific"]["isOpened"] then
                    Config.ShowRequiredItems({
                        [1] = {name = KGCore.Shared.Items["security_card_02"]["name"], image = KGCore.Shared.Items["security_card_02"]["image"]}
                    }, false)
                    ...
                    KGCore.Functions.Progressbar("security_pass", Lang:t("general.validating_bankcard"), math.random(5000, 10000), false, true, {
                        ...
                        TriggerServerEvent('kg-bankrobbery:server:removeBankCard', '02')
                        if copsCalled or not Config.BigBanks["pacific"]["alarm"] then return end
                        TriggerServerEvent("kg-bankrobbery:server:callCops", "pacific", 0, pos)
                        copsCalled = true
                    end, function() -- Cancel
                        StopAnimTask(ped, "anim@gangops@facility@servers@", "hotwire", 1.0)
                ...
                KGCore.Functions.Notify(Lang:t("error.minimum_police_required", {police = Config.MinimumPacificPolice}), "error")
            end
        else
                ...
                if not Config.BigBanks["pacific"]["isOpened"] then
                    ...
                    if hasItem then
                        ...
                        KGCore.Functions.Progressbar("hack_gate", Lang:t("general.connecting_hacking_device"), math.random(5000, 10000), false, true, {
                            ...
                            if success then
                                TriggerServerEvent('kg-bankrobbery:server:setBankState', 'pacific')
                            end
                            if copsCalled or not Config.BigBanks["pacific"]["alarm"] then return end
                            TriggerServerEvent("kg-bankrobbery:server:callCops", "pacific", 0, pos)
                            copsCalled = true
...
CreateThread(function()
    ...
    thermite1Zone:onPlayerInOut(function(inside)
        if inside and not Config.BigBanks["pacific"]["thermite"][1]["isOpened"] then
            ...
            Config.ShowRequiredItems({
                [1] = {name = KGCore.Shared.Items["thermite"]["name"], image = KGCore.Shared.Items["thermite"]["image"]},
            }, true)
        else
    ...
    for k in pairs(Config.BigBanks["pacific"]["lockers"]) do
        if Config.UseTarget then
            exports['kg-target']:AddBoxZone('pacific_coords_locker_'..k, Config.BigBanks["pacific"]["lockers"][k]["coords"], 1.0, 1.0, {
