...
RegisterNetEvent('kg-bankrobbery:UseBankcardA', function()
            ...
            if CurrentCops >= Config.MinimumPaletoPolice then
                if not Config.BigBanks["paleto"]["isOpened"] then
                    ...
                    KGCore.Functions.Progressbar("security_pass", Lang:t("general.validating_bankcard"), math.random(5000, 10000), false, true, {
                        ...
                        TriggerServerEvent('kg-bankrobbery:server:setBankState', 'paleto')
                        TriggerServerEvent('kg-bankrobbery:server:removeBankCard', '01')
                        Config.DoorlockAction(4, false)
                        if copsCalled or not Config.BigBanks["paleto"]["alarm"] then return end
                        TriggerServerEvent("kg-bankrobbery:server:callCops", "paleto", 0, pos)
                        copsCalled = true
                    end, function() -- Cancel
                        StopAnimTask(ped, "anim@gangops@facility@servers@", "hotwire", 1.0)
                ...
                KGCore.Functions.Notify(Lang:t("error.minimum_police_required", {police = Config.MinimumPaletoPolice}), "error")
            end
        else
...
CreateThread(function()
    ...
    bankCardAZone:onPlayerInOut(function(inside)
        ...
        if inside and not Config.BigBanks["paleto"]["isOpened"] then
            Config.ShowRequiredItems({
                [1] = {name = KGCore.Shared.Items["security_card_01"]["name"], image = KGCore.Shared.Items["security_card_01"]["image"]}
            }, true)
        else
            Config.ShowRequiredItems({
                [1] = {name = KGCore.Shared.Items["security_card_01"]["name"], image = KGCore.Shared.Items["security_card_01"]["image"]}
    ...
    for k in pairs(Config.BigBanks["paleto"]["lockers"]) do
        if Config.UseTarget then
            exports['kg-target']:AddBoxZone('paleto_coords_locker_'..k, Config.BigBanks["paleto"]["lockers"][k]["coords"], 1.0, 1.0, {
